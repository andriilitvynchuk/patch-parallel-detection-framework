...
from dronedet.models import DetectionBatchRunner, TrackerRunnerManager
...
class DroneDetPipeline(SimplePipeline):
    def __init__(self, config: Dict[str, Any]):
        ...
        self.tracker_runner_manager = TrackerRunnerManager(config=config["tracker"], global_config=config)
    ...
    def connect_runners(self) -> None:
        ...
        self.detection_batch_runner.add_child(
            self.tracker_runner_manager, dict_keys=["bboxes"], unbatch_keys=["bboxes"]
        )
        self.tracker_runner_manager.add_child(self.visualization_runner_manager, dict_keys=["bboxes_with_tracks"])
